...
d_state_popvote$state_abbreviation <- state.abb[match(d_state_popvote$state, state.name)]
...
```{r, echo=FALSE, warning=FALSE, include=FALSE, message=FALSE}
state_events <- d_campaign_events |>
  left_join(d_state_popvote, by = "state_abbreviation") 
```
```{r, echo=FALSE, warning=FALSE, include=FALSE, message=FALSE}
state_events <-
  state_events |>
  group_by(year.y, state) |>
  filter(party == "DEM") |>
  summarize(d_campaign_events = n()) 
```
...
model2 <- d_state_polls |>
  ...
  rename(September_Results = month_9, 
    ...
    left_join(state_events, by = c("state", "year"))
```

```{r, echo=FALSE, warning=FALSE, include=FALSE, message=FALSE}
reg2 <- lm(D_pv2p ~ September_Results + October_Results + d_campaign_events, 
                data = model2)
```
...
```
```{r}
d_state_polls$state_abbreviation <- state.abb[match(d_state_polls$state, state.name)]

d_state_polls
```

```{r}
state_events$state_abbreviation <- state.abb[match(state_events$state, state.name)]
state_events
```
```{r, echo=FALSE, include=FALSE, warning=FALSE, message=FALSE}
state_poll_24 <- d_state_polls |>
  select(year, state_abbreviation, weeks_left, poll_date, poll_support, party) |>
  filter(month(poll_date) %in% c(9, 10),
         party == "REP", 
         year == 2024) |>
  group_by(year, state_abbreviation, month = month(poll_date)) |>
  ...
  left_join(state_events, by = c("year", "state_abbreviation"))

pred2 <- predict(reg2, state_poll_24, interval = "prediction")
```

```{r, echo=FALSE, warning=FALSE, include=FALSE, message=FALSE}

new_table <- bind_cols(state_poll_24,
  as.data.frame(pred2) |> rename_with(~paste0("D_", .))) |>
...
filter(state %in% c("AZ", "CA", "FL", "GA", "MD", "MI"))
```


# Toss Up States

Now, on to the toss up states. There are some very interesting projections here this time. You will see that I am currently projecting Vice President Harris to flip Florida and Texas, two states which most pollsters have as strongly Republican. If these projections are correct, Harris would _easily_ win the White House. 

```{r}
colnames(state_poll_24)
head(state_poll_24) 
```
