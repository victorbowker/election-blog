...
model1 <- d_state_polls |>
  ...
  filter(month(poll_date) %in% c(8, 9, 10), party == "DEM", year >= 1996) |>
  ...
  select(year, state, month_8, month_9, month_10, D_pv2p) |>
  drop_na() |>
  
  rename(August_Results = month_8,
    September_Results = month_9, 
         October_Results = month_10)

reg1 <- lm(D_pv2p ~ August_Results + September_Results + October_Results, 
                data = model1)
```
